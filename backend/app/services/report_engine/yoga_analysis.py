...
def build_yoga_section(
    ...
    *,
    ...
    if brain_context is not None:
        brain_narrative = brain_context.section_narrative(NarrativeSectionId.YOGAS)
        yoga_evidence = [
            f"{item.title}: {item.summary}"
            for item in brain_context.evidence_for_source(EvidenceSource.YOGAS)
            if item.title and item.summary
        ]
        extra = [line for line in (brain_narrative, *yoga_evidence[:4]) if line]
        if extra:
